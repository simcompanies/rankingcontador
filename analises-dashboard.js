...
function renderEvolucaoBoard(){
  ...
  board.innerHTML = obterTodasDivisoes().map(div => `
    <div class="division" data-div-id="${div.id}" style="--div-accent:var(${div.cor || '--muted'})">
      <div class="division-head"><div><div class="division-title">${div.titulo}</div></div></div>
...
function renderEvolucaoChart(divId, dias){
  ...
  wrap.innerHTML = `
    <svg viewBox="0 0 ${W} ${H}" class="chart-svg" role="img" aria-label="Evolução acumulada da ${div.titulo} no período selecionado">
      ${gradesSvg}
...
function renderHeatmapBoard(){
  ...
  board.innerHTML = obterTodasDivisoes().map((div, i) => `
    <div class="division-title${i>0?' heat-divider':''}" style="color:var(${div.cor || '--muted'});">${div.titulo}</div>
