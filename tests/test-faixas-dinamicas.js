...
const ctx={console,alert:m=>alerts.push(String(m)),confirm:m=>{confirms.push(String(m));return true;},prompt:()=>null,crypto:{randomUUID:()=>Math.random().toString(36).slice(2)},document:{getElementById:()=>null,querySelectorAll:()=>[]},window:null};ctx.window=ctx;
...
alerts.length=0; const rem=ctx.removerFaixa('f1');
...
ok('faixa vazia pode ser removida',ctx.removerFaixa('f1')===true && !vm.runInContext(`state.divisoes.some(d=>d.id==='f1')`,ctx));
