...
const assembled = [semComentariosHtml(read('index.html')), ...moduleFiles.map(f=>semComentariosHtml(read(f)))].join('\n');
...
ok(dup.length===0, 'DOM montado não contém IDs duplicados', JSON.stringify(dup));
